...
import { Trophy, Users, Activity, Target, Zap, Clock, ChevronUp, Star, Medal, ArrowLeft, Crown, Flame, Award, MousePointer2, TrendingUp, Wallet } from "lucide-react";
...
async function getGlobalLeaderboard() {
    ...
    const pipeline: any[] = [
        { $match: { type: 'winnings', status: 'completed' } },
        {
            $group: {
                _id: "$userId",
        ...
        { $sort: { totalEarnings: -1, winCount: -1 } },
        {
            $lookup: {
                from: "users",
                localField: "_id",
                    ...
                    <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-5 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3 md:gap-4 shrink-0">
                            ...
                            <div className="flex items-center gap-2 md:gap-3 bg-indigo-600/10 border border-indigo-500/20 px-3 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl shadow-xl shadow-indigo-600/5">
                                ...
                                <div>
                                    <span className="block text-[7px] md:text-[8px] font-black text-indigo-500/60 uppercase tracking-widest leading-none mb-0.5 md:mb-1">Top Earnings</span>
                                    <span className="block text-sm md:text-xl font-black text-white leading-none tabular-nums">₹{leaderboard[0]?.totalEarnings?.toLocaleString() || 0}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div className="order-1 md:order-2 relative group cursor-pointer animate-in fade-in zoom-in-95 duration-1000">
                                        ...
                                        <div className="relative z-10 bg-gradient-to-r md:bg-gradient-to-b from-indigo-600/20 to-indigo-950/20 backdrop-blur-3xl border border-indigo-500/40 rounded-3xl md:rounded-[64px] p-5 md:p-10 flex flex-row md:flex-col items-center justify-between md:justify-center transition-all duration-500 hover:border-indigo-500/60 h-auto md:h-[360px] shadow-[0_0_30px_rgba(79,70,229,0.15)]">
                                            ...
                                            <div className="flex-1 flex flex-col items-start md:items-center min-w-0 md:w-full space-y-1 md:space-y-0">
                                                ...
                                                <span className="text-[9px] md:text-[10px] font-black text-indigo-400/80 uppercase tracking-widest md:tracking-[0.4em]">Ultimate Earner</span>
                                            </div>
                                            <div className="flex-shrink-0 ml-3 md:ml-0 md:mt-4">
                                                <div className="flex flex-col md:flex-row items-end md:items-center gap-1 md:gap-3 px-3 py-1.5 md:px-6 md:py-2.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30">
                                                    <Wallet className="hidden md:block w-4 h-4 text-emerald-400 shrink-0" />
                                                    <span className="text-xl md:text-2xl font-black text-white tracking-tighter italic leading-none">₹{leaderboard[0].totalEarnings.toLocaleString()}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="order-2 md:order-1 relative group cursor-pointer animate-in fade-in slide-in-from-left-8 duration-1000 delay-300">
                                        ...
                                        <div className="relative z-10 bg-slate-900/60 md:bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-3xl md:rounded-[48px] p-4 md:p-8 flex flex-row md:flex-col items-center justify-between md:justify-center transition-all duration-500 hover:border-slate-400/30 h-auto md:h-[280px] shadow-2xl">
                                            ...
                                            <div className="flex-1 flex flex-col items-start md:items-center min-w-0 md:w-full">
                                                ...
                                                <div className="md:hidden mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-400/10 border border-emerald-400/20">
                                                    <TrendingUp className="w-2.5 h-2.5 text-emerald-400" />
                                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">₹{leaderboard[1].totalEarnings.toLocaleString()}</span>
                                                </div>
                                            </div>
                                            <div className="hidden md:inline-flex mt-auto items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-400/10 border border-emerald-400/20">
                                                <TrendingUp className="w-3 h-3 text-emerald-400 shrink-0" />
                                                <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">₹{leaderboard[1].totalEarnings.toLocaleString()}</span>
                                            </div>
                                            <div className="md:hidden flex-shrink-0 ml-3 text-right">
                                                <span className="text-lg font-black text-white tracking-tighter italic">₹{leaderboard[1].totalEarnings}</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="relative z-10 bg-slate-900/60 md:bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-3xl md:rounded-[48px] p-4 md:p-8 flex flex-row md:flex-col items-center justify-between md:justify-center transition-all duration-500 hover:border-amber-700/30 h-auto md:h-[280px] shadow-2xl">
                                            ...
                                            <div className="flex-1 flex flex-col items-start md:items-center min-w-0 md:w-full">
                                                ...
                                                <div className="md:hidden mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-amber-700/20 border border-amber-700/30">
                                                    ...
                                                    <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest">₹{leaderboard[2].totalEarnings.toLocaleString()}</span>
                                                </div>
                                            </div>
                                            <div className="hidden md:inline-flex mt-auto items-center gap-2 px-3 py-1.5 rounded-full bg-amber-700/20 border border-amber-700/30">
                                                ...
                                                <span className="text-[10px] font-black text-amber-400 uppercase tracking-[0.2em]">₹{leaderboard[2].totalEarnings.toLocaleString()}</span>
                                            </div>
                                            <div className="md:hidden flex-shrink-0 ml-3 text-right">
                                                <span className="text-lg font-black text-white tracking-tighter italic">₹{leaderboard[2].totalEarnings}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="relative z-10 w-full animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
                        <div className="flex items-center justify-between mb-8 px-2">
                            <div className="flex items-center gap-4">
                                ...
                                <h3 className="text-xl font-black text-white tracking-widest uppercase">Wealth & Proficiency Index</h3>
                            </div>
                        ...
                        <div className="md:hidden space-y-4">
                            {leaderboard.map((player: any, index: number) => (
                                <div key={player._id} className="bg-slate-950/40 backdrop-blur-3xl rounded-3xl border border-white/[0.05] p-6 flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-4 min-w-0">
                                        <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-black italic text-sm ${index === 0 ? "bg-yellow-500/10 text-yellow-500" : index === 1 ? "bg-slate-400/10 text-slate-400" : index === 2 ? "bg-amber-700/10 text-amber-700" : "bg-white/[0.03] text-slate-600"}`}>
                                            {index + 1}
                                        </div>
                                    ...
                                    <div className="text-right flex-shrink-0">
                                        <div className="flex items-center gap-2">
                                            <span className="text-xl font-black text-white italic tracking-tighter tabular-nums">₹{player.totalEarnings.toLocaleString()}</span>
                                            <Crown className={`w-4 h-4 ${index === 0 ? "text-yellow-500" : "text-emerald-400"}`} />
                                        </div>
                                        <span className="block text-[8px] font-black text-slate-600 uppercase tracking-widest mt-1">Platform Earnings</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                                ...
                                <table className="w-full border-collapse">
                                    <thead>
                                        <tr className="bg-white/[0.02] border-b border-white/[0.05]">
                                            ...
                                            <th className="px-8 py-6 text-center text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">Total Wins</th>
                                            <th className="px-8 py-6 text-right text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">Accumulated Wealth</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {leaderboard.map((player: any, index: number) => (
                                            <tr key={player._id} className="group relative transition-all duration-300 hover:bg-indigo-600/[0.03]">
                                                <td className="px-8 py-6 text-center">
                                                    <div className={`relative inline-flex items-center justify-center w-10 h-10 rounded-2xl font-black italic text-sm transition-all duration-300 group-hover:scale-110 ${index === 0 ? "bg-yellow-500/10 text-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.2)]" : index === 1 ? "bg-slate-400/10 text-slate-400" : index === 2 ? "bg-amber-700/10 text-amber-700" : "bg-white/[0.03] text-slate-600"}`}>
                                                        {index + 1}
                                                ...
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-5">
                                                        ...
                                                        <div className="flex flex-col min-w-0">
                                                            <span className="text-base font-black text-white tracking-tight group-hover:text-indigo-400 transition-colors truncate">{player.name}</span>
                                                ...
                                                <td className="px-8 py-6 text-center">
                                                    <div className="inline-flex flex-col items-center">
                                                        <div className="flex items-center gap-2 px-4 py-1.5 rounded-xl bg-slate-900 border border-white/5 font-black text-xs text-indigo-400">
                                                            <Trophy className="w-3.5 h-3.5" /> {player.winCount}
                                                        </div>
                                                        <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest mt-2">Tournament Wins</span>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6 text-right">
                                                    <div className="flex flex-col items-end">
                                                        <div className="flex items-center gap-3">
                                                            <div className="flex flex-col items-end">
                                                                <span className="text-2xl font-black text-white italic tracking-tighter tabular-nums leading-none">₹{player.totalEarnings.toLocaleString()}</span>
                                                            </div>
                                                            <div className={`p-2 rounded-lg ${index === 0 ? "bg-yellow-500/10 text-yellow-500" : "bg-emerald-500/10 text-emerald-400"}`}>
                                                                <Wallet className="w-4 h-4" />
                                                            </div>
                                                        </div>
                                                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] mt-2">Gross Profit</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
